...
## 2026-05-06 — [FIX] reference_images relative paths resolve from --folder (PR #138 → alpha)
The fix was already in `_resolve_still_refs` (`settings.folder / r`) and `validate_plan` (`_check_path(..., folder, ...)`). Both correctly resolve relative `reference_images` paths against `--folder`, not the plan file's parent. Added two regression tests covering the split-dir scenario (plan on a drive mount, assets in a local checkout) to prevent future regressions.
**Breaks if:** `parallax validate --plan /some/drive/plan.yaml --folder /local/checkout/` with `reference_images: [asset.png]` (file in --folder) exits 1 or reports a reference_images error.
## 2026-05-06 — [FIX] Linter correctness: clip_path guard, plan-write isolation, regenerate flag, bare-colon validation (PR #134 → alpha)
Four bugs fixed together. (1) `_is_clip_reusable` now checks `Path(clip).exists()` — deleting the file without clearing the YAML field previously blocked re-animation. (2) `_lock_field_in_plan` now reads the on-disk YAML and patches only the targeted scene/field before writing back; the old approach dumped the entire in-memory dict (loaded at run start), silently overwriting any user edits made during a run. (3) New `regenerate: true` per-scene flag: produce clears `still_path`/`clip_path`/`end_frame_path` before any stage runs, then removes the flag — the only safe way to force full re-generation without deleting files. (4) `validate_plan` now pre-checks raw YAML for dict-typed free-text fields (`prompt`, `vo_text`, `motion_prompt`), which indicates a bare-colon YAML misparse, and surfaces an actionable error before Pydantic runs. Index in the error is `scenes[?]` (not the concrete number) because a `ScannerError` prevents parsing the document far enough to determine which scene caused it.
**Breaks if:** a produce run with a scene that had `regenerate: true` still keeps its locks after the run; or `parallax validate --plan` on a file with `prompt: key: value` (unquoted) exits 0 instead of 1.
...
## 2026-05-06 — [FIX] Scene aligner: duration_s cascade and shared-vocabulary word stealing (PR #129 → alpha)
Two bugs caused incorrect scene cut timing. Bug 1: `align_scenes_obj` never received `duration_s` pins from plan.yaml (they live only on `SceneSpec`, not `SceneRuntime`), so the word cursor wasn't advanced past the pinned boundary before the next scene's word search — only the post-hoc `_apply_timing_overrides` moved timestamps, leaving the word assignment wrong. Fix: `stage_align` now merges `duration_s` from plan scenes into dicts before calling `align_scenes`, and `align_scenes_obj` advances the cursor to the first word at/after `word_start + duration_s`. Bug 2: `_find_scene_end` searched backwards in a `2× expected + 5` word window, which could reach into the next scene and find a shared word (e.g. "back", "before") before finding the correct one. Fix: `align_scenes_obj` pre-computes proportional `window_cap` per scene from total word distribution and passes it to `_find_scene_end`. Also: `duration_s` now rounded to 2dp; short-scene warning fires when detected duration < 50% of expected.
**Breaks if:** `parallax produce` with `duration_s` pinned on scene N shows scene N+1 with the wrong words in captions; or adjacent scenes with shared vocabulary still get wrong durations after this fix ships.
...
## 2026-05-05 — [CHANGED] Add parallax validate subcommand (PR #90 → alpha)
`parallax validate --brief <path> --folder <dir>` (and `--plan`) runs schema parse, provided-asset existence, `image_refs` resolution, and locked-path checks without generating any assets or spending any credits. Exits 0 if valid, 1 if any errors. JSON emitted to stdout with `valid`, `errors`, and `warnings` keys. `product_ref` assets emit a warning (no wiring yet — see #83). 13 new unit tests.
**Breaks if:** `parallax validate --brief <valid-brief.yaml> --folder <folder>` exits non-zero or does not print JSON with a `valid` key; or a brief with a missing `provided` asset path exits 0 instead of 1.
...
## 2026-05-05 — [CHANGED] Centralize generated stills and clips in parallax/assets/ (PR #81 → alpha)
Generated stills (`scene_NN_still.png`) and animated clips (`scene_NN_animated.mp4`) now land in a flat `parallax/assets/` directory with deterministic scene-based names rather than inside versioned `parallax/output/vN/` dirs. Regenerating a scene overwrites the asset in place; unchanged scenes' assets remain untouched. Versioned `vN/` dirs now contain only the final assembled `.mp4`, manifest, plan snapshot, cost.json, and run.log. Existing plans with old `vN`-relative locked paths continue to resolve correctly (no migration). A per-scene temp subdir (`_anim_NN/`) is used during animation to avoid hash collisions between concurrent scenes with identical prompts (would otherwise race on the same deterministic filename). Verify suite `expected.yaml` fixtures had their `stages.stills` blocks removed since stills no longer live under `out_dir` — resolution validation still covered end-to-end by the final video check.
**Breaks if:** After `parallax produce`, stills appear inside `parallax/output/vN/stills/` instead of `parallax/assets/`; or a second produce run regenerates stills that were already locked rather than reporting "reusing scene_NN_still.png".

## 2026-05-05 — [FIX] Strip TTS emotional tags from mock words and stage log (PR #80 → alpha)
`[casually]`, `[dramatically]` etc. in `vo_text` leaked into two places: (1) `_mock_voiceover` split `vo_text` verbatim, so bracket tokens appeared as words in `vo_words.json` and rendered in captions during test-mode runs; (2) the still-generation `stage.log` line printed raw `vo_text`, so logs showed `[casually] I saw an article...` instead of clean speech text. Fixed by stripping `\[[^\]]*\]` before the split in `_mock_voiceover` and before the display slice in `stages.py`. `assembly.py` (`align_scenes`) already had equivalent stripping on alpha.
**Breaks if:** `PARALLAX_TEST_MODE=1 parallax produce` on a brief with `[emotional]` tags produces a `vo_words.json` containing any word entry starting with `[`, or stage log lines show bracket-prefixed scene text.
...
## 2026-05-05 — [FIX] Dry-run mock stubs rejected by live produce runs (PR #72 → alpha)
`stage_stills` and `stage_animate` now detect locked `still_path`/`clip_path` values that point to mock files (`mock_*.png`, `mock_video_*.mp4`) and regenerate instead of reusing them when `settings.mode == REAL`. `shim.is_mock_asset(path)` is the single detection point — filename starts with `mock_`. TEST mode retains the existing hash-cache reuse behavior. Root cause: `_lock_field_in_plan` writes the mock path back into plan.yaml during a dry run; subsequent live runs found the field set and skipped generation entirely, silently baking placeholder stills into the final video.
**Breaks if:** `PARALLAX_TEST_MODE=1 parallax produce` followed immediately by `parallax produce` (no test mode, same folder) logs `reusing mock_*.png` instead of `dry-run mock detected — regenerating`.
...
## 2026-05-04 — [CHANGED] Inject ProductionMode into run_plan instead of mutating os.environ (PR #62 → alpha)
`verify_suite.run_case` previously set/restored `os.environ["PARALLAX_TEST_MODE"]` to drive test mode — fragile under parallel test runs. Now `run_plan` accepts `mode: ProductionMode | None`; `resolve_settings` threads it through to `_resolve_mode`. `shim.is_test_mode()` checks a `contextvars.ContextVar` (`_test_mode_override`) set inside `run_plan` before falling back to env, so all provider/media callers pick up the correct mode without any env mutation. `PARALLAX_TEST_MODE=1` remains valid for the CLI path.
**Breaks if:** `parallax verify suite tests/integration/` fails when `PARALLAX_TEST_MODE` is absent from env; or `test_run_case_test_mode_without_env_var` fails; or a real produce run ignores the env var and hits stubs.
...
## 2026-05-03 — [CHANGED] Replace internal JSON-string APIs with typed Python objects (PR #54 → main)
Added `_obj` / `_data` / `_dict` variants for every internal function that previously passed JSON strings across module boundaries: `align_scenes_obj`, `ken_burns_assemble_obj`, `assemble_clip_video_obj` (assembly.py); `write_manifest_data`/`read_manifest_data` (manifest.py); `generate_voiceover_dict` (voiceover.py). `stages.py` now calls the object-level variants directly, eliminating all `json.dumps`/`json.loads` roundtrips in the hot path. JSON-string wrappers (`align_scenes`, `ken_burns_assemble`, etc.) kept as thin forwards for CLI/external callers — existing tests pass unchanged. Removed `rt["aligned_json"]` from `_runtime`; `stage_assemble` reads `rt["aligned"]` directly. 7 new tests assert object-level return types and equivalence with JSON wrappers.
**Breaks if:** `stage_assemble` raises `KeyError: 'aligned_json'`, or any of the new `_obj`/`_data`/`_dict` functions return a JSON string instead of a Python object.
