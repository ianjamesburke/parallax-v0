...
SYSTEM_PROMPT = (
    ...
    "### Ken Burns Pipeline\n\n"
    "1. **Scan** — call `scan_project_folder`. Extracts script_text and character_image_path.\n"
    ...
    "   Default voice='george', speed=1.1.\n"
    ...
    "   and default resolution='1080x1920' (vertical). Use '1920x1080' for landscape.\n"
    "7. **Burn captions** — call `burn_captions` with the draft video path and the words_path from step 4. "
    "   Default: caption_style='anton', fontsize=70, words_per_chunk=1.\n"
    ...
    "9. **Report** — tell the user the final video path and a one-line summary of what was made.\n\n"
    ...
    "1. **Scan** — `scan_project_folder` returns `mode`, `script_text`, and `clips` (dict of number-string → path).\n"
    "2. **Parse scenes** — read `script_text`. Each `[NNN]` or `[NNN-NNN]` marker on a line defines one scene:\n"
    "   - `clip_paths`: list of file paths from `clips` dict for each number in the range\n"
    "     e.g. `[002-004]` → [clips['2'], clips['3'], clips['4']]\n"
    "   - `vo_text`: text on that line, stripped of the `[NNN]` marker\n"
    "   - `index`: integer starting at 0\n"
    ...
    "   Strip `f***` forms to `f***` (leave as-is for ElevenLabs).\n"
    "3. **Generate voiceover** — call `generate_voiceover` with the clean VO script.\n"
    "4. **Align scenes** — call `align_scenes` with scenes JSON and words from step 3.\n"
    "5. **Assemble clips** — call `assemble_clip_video` with the aligned scenes JSON and audio_path.\n"
    "6. **Burn captions** — call `burn_captions` with the assembled video and words_path.\n"
    "   Default: caption_style='anton', fontsize=70, words_per_chunk=1.\n"
    "   If captions: skip is set in overrides, omit this step entirely.\n"
    "7. **Burn headline** — only if a headline is specified in overrides. "
    "   Always pass `end_time_s` as the first scene's `end_s` so the headline shows only during the intro.\n"
    "8. **Report** — tell the user the final video path and a one-line summary.\n\n"
    "## Pipeline overrides\n\n"
